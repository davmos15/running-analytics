...
class FirebaseService {
  ...
  // Calculate activity-level metrics from streams (delegates to segmentEngine)
  calculateActivityMetrics(streams) {
    return segmentEngine.calculateActivityMetrics(streams);
  }
  ...
  // Find best segments using GPS streams (delegates to segmentEngine)
  async findBestSegmentsFromStreams(activity, streams) {
    return segmentEngine.findBestSegmentsFromStreams(activity, streams);
  }

  // Calculate enhanced metrics for a segment from stream data (delegates)
  calculateSegmentMetrics(segment, heartRateStream, cadenceStream, altitudeStream, wattsStream = null, velocityStream = null) {
    return segmentEngine.calculateSegmentMetrics(
      segment, heartRateStream, cadenceStream, altitudeStream, wattsStream, velocityStream
    );
  }

  // Sliding window algorithm to find fastest segment (delegates to segmentEngine)
  ...
  async getRunningStatsSummary() {
    try {
      return await this.getCachedQuery('running_stats_summary', async () => {
        const activities = await this.getActivities();
        const runActivities = activities.filter(activity => 
          activity.type && ['Run', 'TrailRun', 'VirtualRun'].includes(activity.type)
        );
